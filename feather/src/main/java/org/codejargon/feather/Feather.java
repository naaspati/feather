...
public class Feather {
	 ...
	 * Constructs Feather with configuration modules
	 */
	public static Feather with(Map<Class, Class> bindings, Iterable<?> modules) {
		if(bindings.isEmpty())
			return new Feather(modules);
		else 
			return new Feather2(bindings, modules);
	}

	private static class Feather2 extends Feather {
		Map<Class, Class> bindings;
		
		private Feather2(Map<Class, Class> bindings, Iterable<?> modules) {
			super(modules);
		...
		<T> Provider<T> provider(Key<T> key, Set<Key> chain) {
			...
			if(c == null)
				...
				return super.provider(Key.of(c, key), chain);
		}
	}
	...
	<T> Provider<T> provider(final Key<T> key, Set<Key> chain) {
